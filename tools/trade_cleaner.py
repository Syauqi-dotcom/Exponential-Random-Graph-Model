...
def get_country_index_dicts(dataframe):
    ...
    index_to_country = {v: k for k, v in country_to_index.items()}
    return country_to_index, index_to_country, all_countries

def main():
    """
    sample usage: python trade_cleaner.py <uncleaned_file_path> <cleaned_file_path>
    """
    if len(sys.argv) != 3:
        print("Mana path datanya dan nama output filenya mau apa?")
        exit()
