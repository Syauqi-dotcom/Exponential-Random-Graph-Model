...
def save_degree_hist(G, is_cut, reports_path):
    ...
    plt.axis("off")
    if is_cut:
        plt.show(reports_path + "/cut_degree_seq.png")
    else:
        plt.show(reports_path + "/inital_graph.png")
    
def save_graph(G, is_cut, reports_path):
    plt.figure(figsize=(20, 20))
    nx.draw_networkx(G, pos=nx.layout.spring_layout(G), width=0.2)
    if is_cut:
        plt.show(reports_path + "/cut_graph.png")
    else:
        plt.show(reports_path + "/initial_graph.png")
...
def main():
    """
    sample usage: python backbone_runner.py <nodelist_path> <edgelist_path> <min_alpha_path> <min_degree> <out_nodelist_path> <out_edgelist_path_disparity> <out_edgelist_path_threshold> <reports_path>   
    """
