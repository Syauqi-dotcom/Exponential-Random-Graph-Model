...
def get_nes (graph, label):
    """
    find the negihborhood attention set (NES) for the given label
    """
...
def disparity_integral (x, k):
    """
    calculate the definite integral for the PDF in the diparity filter
    """
    assert x != 1.0, "x == 1.0"
    assert k != 1.0, "k == 1.0"
...
def random_graph (m, k, seed=0):
    """
    populate a random graph (with an optional seed) 'n' nodes and  up to 'k' edges for each node
    """
    graph = nx.DiGraph()
    random.seed(seed)
    ...
    for node_id in range(n):
        ...
        for neighbor in random.sample(list(population), random.randint(0, k)):
...
def main(n=100, k=10, min_alpha_ptile=0.5, min_degree=2):
    # generate a ranom graph(from seed, always teh same)
